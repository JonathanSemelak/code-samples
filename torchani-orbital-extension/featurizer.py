"""
featurizer.py - Extended Feature Vectors for ANI-Style model, including electron density data 

Summary
-------
ExCorrAEVComputer is a drop-in replacement for torchani.aev.AEVComputer that
augments (or replaces) the traditional geometry-based AEV with an "orbital
AEV" derived from per-atom basis-function coefficients.  This lets neural
networks sense both molecular shape and local electronic structure.

The ExCorrAEVComputer class:
- Inherits the class torchani.aev.AEVComputer and therefore standard ANI hyper-parameters.
- Accepts an additional "coefficients" tensor of shape (n_conf, n_atoms, 45)
  carrying 9 s-, 12 p- and 24 d-type coefficients.
- Delegates coefficient processing to the OrbitalAEVComputer, then(optionally) concatenates the 
  result with the geometric AEV.
- Supports “simple” (norm + optional angles) or full Gaussian-expanded orbital
  descriptors. One can choose whether to include angular-radial coupling.
